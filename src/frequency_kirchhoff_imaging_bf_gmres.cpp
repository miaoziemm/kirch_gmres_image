...
#ifdef KIRCH_GLOBAL_PRECONDITIONED_GMRES
...
#elif defined(KIRCH_DIRECT_KIRCHHOFF_GMRES)
constexpr const char* kProgramName = "frequency_kirchhoff_imaging_direct_gmres";
constexpr const char* kMethodDescription =
    "direct Kirchhoff summation, 1D datum -> 2D block";
#else
constexpr const char* kProgramName = "frequency_kirchhoff_imaging_bf_gmres";
constexpr const char* kMethodDescription =
    "rectangular ButterflyPACK, 1D datum -> 2D block";
#endif
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
void direct_apply_pair_depth_major(
    const se::huygens::OneWayKernelData& kernel,
    const fki::Complex* source_input,
    const fki::Complex* receiver_conjugate_input,
    int nx,
    int depth_count,
    std::vector<fki::Complex>& source_output,
    std::vector<fki::Complex>& receiver_conjugate_output)
{
    if (source_input == nullptr || receiver_conjugate_input == nullptr ||
        nx <= 0 || depth_count <= 0 ||
        kernel.rows != nx * depth_count) {
        throw std::invalid_argument(
            "direct Kirchhoff block dimensions or input pointers are invalid");
    }

    source_output.assign(static_cast<std::size_t>(kernel.rows),
                         fki::Complex(0.0f, 0.0f));
    receiver_conjugate_output.assign(static_cast<std::size_t>(kernel.rows),
                                     fki::Complex(0.0f, 0.0f));

    // OneWayKernelData uses x-major rows (depth is fastest), while the rest of
    // the production imaging path uses depth-major storage.  Compute the exact
    // direct sum and place each row directly into its depth-major destination.
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
    for (int row = 0; row < kernel.rows; ++row) {
        fki::Complex source_sum(0.0f, 0.0f);
        fki::Complex receiver_sum(0.0f, 0.0f);
        for (int source = 0; source < kernel.sources; ++source) {
            const fki::Complex entry = kernel.entry(row, source);
            source_sum += entry * source_input[source];
            receiver_sum += entry * receiver_conjugate_input[source];
        }

        const int ix = row / depth_count;
        const int iz_local = row - ix * depth_count;
        const std::size_t packed =
            static_cast<std::size_t>(iz_local) * nx + ix;
        source_output[packed] = source_sum;
        receiver_conjugate_output[packed] = receiver_sum;
    }
}
#endif
...
void accumulate_image_batch(
    ...
    const std::vector<float>& depth_partition_weights,
    ...
    std::vector<float>& illumination)
{
    ...
    if (depth_partition_weights.size() != target_depths.size()) {
        throw std::invalid_argument(
            "image partition weights do not match target depths");
    }
    ...
    for (std::size_t ifrequency_local = 0;
        ...
        const float frequency_weight = axis.imaging_weights[ifrequency];
        ...
        for (int iz_batch = 0; iz_batch < depth_count; ++iz_batch) {
            ...
            const float weight = frequency_weight *
                depth_partition_weights[static_cast<std::size_t>(iz_local)];
...
/* -------------------------------------------------------------------------
 * GMRES external absorbing padding
 *
 * The original correction system was assembled directly on the physical
 * propagation block, so build_helmholtz() placed its absorbing rows/columns
 * inside the actual Kirchhoff block.  For the PML-only revision requested
 * here, every GMRES block is first extended by absorbing_rows samples on all
 * four sides.  The artificial samples use constant-edge continuation.  The
 * Helmholtz damping profile is then assembled on this extended grid, so all
 * non-zero damping lies outside the original physical block.
 *
 * Storage follows matlab_gmres::index(): z is the fast dimension.
 * ------------------------------------------------------------------------- */
template <typename T>
std::vector<T> pad_gmres_block_constant_edge(
    const std::vector<T>& physical,
    int nz, int nx, int pad)
{
    if (nz <= 0 || nx <= 0 || pad < 0 ||
        static_cast<int>(physical.size()) != nz * nx) {
        throw std::invalid_argument("invalid GMRES block for external padding");
    }
    if (pad == 0) return physical;

    const int nz_padded = nz + 2 * pad;
    const int nx_padded = nx + 2 * pad;
    std::vector<T> padded(
        static_cast<std::size_t>(nz_padded) * nx_padded);

    for (int ixp = 0; ixp < nx_padded; ++ixp) {
        const int ix = std::min(nx - 1, std::max(0, ixp - pad));
        for (int izp = 0; izp < nz_padded; ++izp) {
            const int iz = std::min(nz - 1, std::max(0, izp - pad));
            padded[static_cast<std::size_t>(
                matlab_gmres::index(izp, ixp, nz_padded))] =
                physical[static_cast<std::size_t>(
                    matlab_gmres::index(iz, ix, nz))];
        }
    }
    return padded;
}
...
void correct_complete_block(
    ...
    const matlab_gmres::PreparedSystem& system, float frequency,
    const matlab_gmres::Parameters& parameters,
    ...
    const char* field_name, std::ofstream& metrics_stream)
{
    ...
    const int pad = parameters.absorbing_rows;
    const int nz_padded = nz + 2 * pad;
    const int nx_padded = nx + 2 * pad;

    if (static_cast<int>(values.size()) != nz * nx) {
        throw std::invalid_argument(
            "GMRES correction requires one complete frequency block");
    }

    /*
     * Complex-frequency convention:
     *
     * The Kirchhoff propagator used by this program stores the downward
     * wavefield with the opposite complex-frequency convention from the
     * MATLAB-style Helmholtz operator
     *
     *     L + k^2 (1 + i eta).
     *
     * Convert once before constructing the equivalent interface source and
     * convert back after GMRES.  Without this conversion, additional GMRES
     * iterations increasingly rotate the block toward the opposite phase
     * convention and make inter-block discontinuities more visible.
     */
    std::vector<std::complex<double>> initial_physical(
        static_cast<std::size_t>(nz) * nx);
    for (int ix = 0; ix < nx; ++ix) {
        for (int iz = 0; iz < nz; ++iz) {
            const int mi = matlab_gmres::index(iz, ix, nz);
            const fki::Complex value =
                values[static_cast<std::size_t>(iz) * nx + ix];
            initial_physical[static_cast<std::size_t>(mi)] =
    ...
    /*
     * Put the absorbing layer completely outside the physical propagation
     * block.  The HPP mask is zero only in this artificial four-side pad.
     */
    const std::vector<std::complex<double>> initial_padded_std =
        pad_gmres_block_constant_edge(
            initial_physical, nz, nx, pad);

    matlab_gmres::Vector initial_padded(nz_padded * nx_padded);
    for (int i = 0; i < initial_padded.size(); ++i) {
        initial_padded[i] =
            initial_padded_std[static_cast<std::size_t>(i)];
    }

    if (system.mask.size() != initial_padded.size()) {
        throw std::runtime_error(
            "padded GMRES system size does not match padded Kirchhoff block");
    }

    std::vector<matlab_gmres::IterationMetric> metrics;
    const matlab_gmres::Vector corrected_padded =
        matlab_gmres::correct(
            system, initial_padded, parameters, &metrics);

    /*
     * Crop directly back to the physical block.
     *
     * IMPORTANT: do NOT apply the old collar/cosine transition here.
     * The PML is already outside the physical block.  Keeping the old
     * transition would freeze the top/bottom physical rows at the original
     * Kirchhoff field while the block interior converges toward the Helmholtz
     * solution, which is exactly the behavior that becomes increasingly
     * blocky when the GMRES iteration count is increased.
     */
    for (int ix = 0; ix < nx; ++ix) {
        for (int iz = 0; iz < nz; ++iz) {
            const int mpi = matlab_gmres::index(
                iz + pad, ix + pad, nz_padded);
            const std::complex<double> value =
                std::conj(corrected_padded[mpi]);
    ...
    // Source and receiver corrections run concurrently.  Serialize only the
    // diagnostic write, not either sparse GMRES solve.
#ifdef _OPENMP
#pragma omp critical(gmres_metrics_output)
#endif
    {
        for (const auto& m : metrics) {
            metrics_stream
                << shot << ',' << block << ',' << frequency << ','
...
int main(int argc, char** argv)
{
    ...
    try {
...
#else
            fki::parse_common_options(kProgramName);
#endif
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
        const int direct_progress = huygens_cli::optional_int("direct_progress", 1);
        const int direct_progress_every =
            huygens_cli::optional_int("direct_progress_every", 1);
#endif
        matlab_gmres::Parameters correction;
        const int gmres_enable =
            huygens_cli::optional_int("gmres_enable", 1);
#ifdef KIRCH_GLOBAL_PRECONDITIONED_GMRES
        const bool local_gmres_enable = false;
#else
        const bool local_gmres_enable = gmres_enable != 0;
#endif
        ...
        correction.absorbing_rows = huygens_cli::optional_int("gmres_nabs", 25);
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
        if ((direct_progress != 0 && direct_progress != 1) ||
            direct_progress_every < 1) {
            throw std::invalid_argument(
                "invalid direct Kirchhoff progress parameters");
        }
#endif
        if ((gmres_enable != 0 && gmres_enable != 1) ||
            correction.outer_iterations < 1 || correction.gmres_restart < 1 ||
            correction.gmres_cycles < 1 || correction.absorbing_rows < 0 ||
            !(correction.gmres_tolerance > 0.0) || !(correction.damp_max >= 0.0)) {
            throw std::invalid_argument(
                "invalid MATLAB GMRES parameters; gmres_enable must be 0 or 1 "
                "and gmres_outer must be >= 1");
        }
#ifdef KIRCH_GLOBAL_PRECONDITIONED_GMRES
        ...
        std::cout
            << "GMRES correction     : "
            << (gmres_enable != 0 ? "enabled" : "disabled") << '\n'
            << "GMRES outer loops    : " << correction.outer_iterations << '\n'
            << "GMRES restart        : " << correction.gmres_restart << '\n'
            << "GMRES cycles/outer   : " << correction.gmres_cycles << '\n';
#endif

#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
        std::cout
            << "Propagation backend   : direct Kirchhoff summation (no ButterflyPACK)\n"
            << "Direct RHS strategy  : one kernel entry updates source + receiver pair\n"
            << "Direct parallelism   : OpenMP over target rows\n"
            << "Direct progress      : enable=" << direct_progress
            << ", every=" << direct_progress_every << " frequency(s)\n"
        ...
        std::cout
            ...
            << "BPACK RHS strategy   : apply_many(source + receiver for all selected shots)\n"
            << "Scheme-A seam        : enable=" << seam_enable
            << ", amp_floor=" << seam_amp_rel_floor
            << ", scale_cap=" << seam_scale_cap
            << ", cache_cap=" << seam_cache_max_mb << " MB\n";
        ...
        for (std::size_t iblock = 0;
            ...
            try {
                ...
                if (seam_enable != 0 && iblock > 0 &&
                    info.overlap_rows > 0) {
                    throw std::runtime_error(
                        "Scheme A requires one-way traveltime tables that "
                        "include the propagation-overlap rows. Regenerate "
                        "them with travel_time_solver_oneway "
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
                throw std::runtime_error(
                    "direct Kirchhoff imaging requires every lateral grid point");
#else
                throw std::runtime_error(
            ...
            std::vector<float> gmres_velocity;
            int gmres_nz = target_depth_count;
            int gmres_nx = model.nx;
            if (iblock != 0 && local_gmres_enable) {
                std::vector<float> gmres_velocity_physical(
                    static_cast<std::size_t>(target_depth_count) * model.nx);
                for (int ix = 0; ix < model.nx; ++ix) {
                    for (int iz = 0; iz < target_depth_count; ++iz) {
                        gmres_velocity_physical[static_cast<std::size_t>(
                            matlab_gmres::index(iz, ix,
                                                target_depth_count))] =
                            model.velocity[model.index(
                                ix, geometry.target_iz[
                                        static_cast<std::size_t>(iz)])];
                    }
                }

                // Move the PML outside the physical propagation block by
                // extending velocity with constant-edge values on all four
                // sides.  build_helmholtz() is unchanged; its absorbing
                // rows/columns now fall entirely inside this artificial pad.
                const int gmres_pad = correction.absorbing_rows;
                gmres_velocity = pad_gmres_block_constant_edge(
                    gmres_velocity_physical,
                    target_depth_count, model.nx, gmres_pad);
                gmres_nz = target_depth_count + 2 * gmres_pad;
                gmres_nx = model.nx + 2 * gmres_pad;
            }
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
            std::cout
                << "DIRECT block " << block.id
                << ": matrix="
                << (static_cast<std::size_t>(target_depth_count) * model.nx)
                << 'x' << bpack_columns
                << ", frequencies=" << axis.frequencies.size()
                << ", RHS/frequency=" << (uses_butterfly ? 2 * states.size() : 0)
                << ", GMRES tile=1freq x " << target_depth_count << "depth"
                << (uses_butterfly
                        ? ""
                        : ", phase-shift block (direct integral skipped)")
                << std::endl;
#else
            std::cout
                ...
                << (uses_butterfly
                        ...
                        : target_depth_count)
                << ", RHS/operator="
                << (uses_butterfly ? 2 * states.size() : 0)
                << ", GMRES tile=1freq x " << target_depth_count << "depth"
                << (uses_butterfly
                        ? ""
...
#ifndef KIRCH_DIRECT_KIRCHHOFF_GMRES
...
#pragma omp parallel for schedule(static)
            ...
            for (std::size_t ifrequency = 0;
                ...
                if (uses_butterfly) {
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
                    const auto direct_started = fki::Clock::now();
                    for (std::size_t ishot = 0; ishot < states.size(); ++ishot) {
                        const std::size_t input_offset =
                            ifrequency * static_cast<std::size_t>(model.nx);
                        direct_apply_pair_depth_major(
                            kernel,
                            states[ishot].source.data() + input_offset,
                            states[ishot].receiver_conjugate.data() + input_offset,
                            model.nx,
                            target_depth_count,
                            propagated_depth_major[2 * ishot],
                            propagated_depth_major[2 * ishot + 1]);
                    }
                    const double direct_wall = std::chrono::duration<double>(
                        fki::Clock::now() - direct_started).count();
                    block_bpack_apply_seconds += direct_wall;

                    if (direct_progress != 0 &&
                        (ifrequency == 0 ||
                         ifrequency + 1 == axis.frequencies.size() ||
                         (ifrequency % static_cast<std::size_t>(
                              direct_progress_every) == 0))) {
                        std::cout
                            << "    [DIRECT apply DONE] block=" << block.id
                            << " freq=" << (ifrequency + 1) << '/'
                            << axis.frequencies.size()
                            << " f=" << frequency << " Hz"
                            << " matrix=" << kernel.rows << 'x' << kernel.sources
                            << " apply=" << direct_wall << " s"
                            << " cum_direct=" << block_bpack_apply_seconds << " s"
                            << std::endl;
                    }
#else
                    ...
                    rhs_inputs.reserve(2 * states.size());
                    for (std::size_t ishot = 0; ishot < states.size(); ++ishot) {
                        rhs_inputs.push_back(boundary_frequency(
                            states[ishot].source, ifrequency, model.nx));
                        rhs_inputs.push_back(boundary_frequency(
                    ...
                    for (std::size_t ichunk = 0;
                        ...
                        if (chunk_rows_out.size() != 2 * states.size()) {
                            throw std::runtime_error(
                ...
                std::unique_ptr<matlab_gmres::PreparedSystem> gmres_system;
                if (uses_butterfly && local_gmres_enable) {
                    gmres_system = std::make_unique<
                        matlab_gmres::PreparedSystem>(
                            matlab_gmres::prepare_system(
                                gmres_velocity,
                                gmres_nz,
                                gmres_nx,
                                model.dz,
                ...
                for (std::size_t ishot = 0; ishot < states.size(); ++ishot) {
                    ...
                    if (!uses_butterfly) {
                        ...
                        pack_frequency_batch(
                            states[ishot].source, ifrequency, 1,
                            model.nx, packed_source);
                        pack_frequency_batch(
                        ...
                        phase_shift_first_block(
                            ...
                            frequency, packed_source, 0, source_output);
                        phase_shift_first_block(
                            model, block, geometry.target_iz,
                            frequency, packed_receiver, 0, receiver_output);
                    } else {
                        source_output =
                            std::move(propagated_depth_major[2 * ishot]);
                        receiver_output =
                            std::move(propagated_depth_major[2 * ishot + 1]);

                        if (local_gmres_enable) {
                            // Source and receiver are independent right-hand sides
                            // over the same immutable prepared Helmholtz system.
#ifdef _OPENMP
...
#pragma omp section
                                ...
                                correct_complete_block(
                                    model, geometry.target_iz, *gmres_system,
...
#pragma omp section
...
#else
                    // Image the raw block solution.  The only overlap weighting
                    // applied to the image is the depth partition of unity.
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
                if (direct_progress != 0 && uses_butterfly &&
                    (((ifrequency + 1) %
                          static_cast<std::size_t>(direct_progress_every)) == 0 ||
                     ifrequency + 1 == axis.frequencies.size())) {
                    std::cout
            ...
            timing_values[static_cast<std::size_t>(column) * metrics + 1] =
                static_cast<float>(block_bpack_apply_seconds);
            timing_values[static_cast<std::size_t>(column) * metrics + 2] =
                static_cast<float>(block_post_seconds);
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
            std::cout
                << "  direct_Kirchhoff=" << block_bpack_apply_seconds << " s"
                << ", GMRES/image=" << block_post_seconds << " s"
                << ", direct+GMRES/correlation=" << block_apply_seconds << " s"
                << ", frequencies=" << axis.frequencies.size()
                << ", target_points=" << geometry.targets.size()
                << '\n';
#else
            std::cout
        ...
        // ------------------------------------------------------------------
        const auto global_started = fki::Clock::now();
        ...
        for (std::size_t ifrequency = 0;
             ifrequency < axis.frequencies.size(); ++ifrequency) {
            const float frequency = axis.frequencies[ifrequency];
            std::unique_ptr<gpg::PreparedSystem> system;
            if (gmres_enable != 0) {
                const auto factor_started = fki::Clock::now();
                system = std::make_unique<gpg::PreparedSystem>(
                    gpg::prepare_system(
                        global_smoothed_velocity, model.nz, model.nx,
                        model.dz, model.dx, frequency, prepared_parameters));
            ...
            for (std::size_t ishot = 0; ishot < states.size(); ++ishot) {
                const int source_ix = nearest_global_source_ix(
                    model, states[ishot].shot_x);
                gpg::Vector source_field = build_global_source_aexp(
                    model, source_traveltimes[ishot], frequency,
                    source_spectrum[ifrequency], source_ix,
                    global_source_regularization);
                gpg::Vector receiver_field = load_global_receiver(
                ...
                if (gmres_enable != 0) {
                    ...
                    const gpg::Vector source_rhs = gpg::make_source_rhs(
                        *system, source_ix,
                        gpg::Complex(
                            static_cast<double>(source_spectrum[ifrequency].real()),
                            static_cast<double>(source_spectrum[ifrequency].imag())),
                        model.dx, model.dz);
                    const gpg::Vector receiver_rhs =
                        gpg::receiver_equivalent_source(
                            *system, receiver_field,
                            global_correction.receiver_mask_rows);
                    const gpg::Vector receiver_guess =
                        gpg::apply_receiver_mask(
                            *system, receiver_field,
                            global_correction.receiver_mask_rows);

                    gpg::Metric source_metric;
                    gpg::Metric receiver_metric;
                    source_field = gpg::solve(
                        *system, source_rhs, source_field, &source_metric);
                    receiver_field = gpg::solve(
                        *system, receiver_rhs, receiver_guess, &receiver_metric);
                    global_solve_seconds += std::chrono::duration<double>(
                        fki::Clock::now() - solve_started).count();

                    correction_stream
                        << shots[ishot] << ",-1," << frequency
                ...
                const float frequency_weight =
                    axis.imaging_weights[ifrequency];
                for (std::size_t grid = 0; grid < grid_size; ++grid) {
                    const gpg::Complex us =
                        source_field[static_cast<Eigen::Index>(grid)];
                    const gpg::Complex ur =
                        receiver_field[static_cast<Eigen::Index>(grid)];
                    image[grid] += frequency_weight *
                        static_cast<float>((us * ur).real());
                    illumination[grid] += frequency_weight *
                        static_cast<float>(std::norm(us));
                }
            }
            ...
            if (bpack_progress != 0 &&
                ...
                std::cout
                    ...
                    << (ifrequency + 1) << '/' << axis.frequencies.size()
                    << " f=" << frequency << " Hz, iterations="
                    << (gmres_enable != 0
                            ? global_correction.iterations
                            : 0)
                    << '\n';
            }
        }
        ...
        se::huygens::write_image_rsf(
            ...
            {{"frequency_min_hz", axis.frequencies.front()},
             ...
             {"seam_enable", static_cast<float>(seam_enable)},
             {"seam_amp_rel_floor", seam_amp_rel_floor},
             {"seam_scale_cap", seam_scale_cap}});
#else
            kProgramName,
            {{"frequency_min_hz",
        ...
        se::huygens::write_timing_rsf(
            ...
            {"filter_setup_seconds",
             "direct_kirchhoff_seconds",
             "gmres_image_seconds",
             "direct_gmres_image_seconds",
             "frequency_count",
             "target_point_count",
             "datum_point_count",
             "unused_7",
            ...
            {{"fft_seconds", static_cast<float>(fft_seconds)},
             ...
             {"total_direct_kirchhoff_seconds",
              static_cast<float>(total_propagation_seconds)},
...
#ifdef KIRCH_DIRECT_KIRCHHOFF_GMRES
        std::cout
            << "Direct Kirchhoff + restarted GMRES imaging completed.\n"
            << "FFT/data preparation:  " << fft_seconds << " s\n"
            << "Filter setup:          " << total_filter_seconds << " s\n"
            << "Direct Kirchhoff sum:  " << total_propagation_seconds << " s\n"
            << "Direct + GMRES/image:  " << total_apply_seconds << " s\n"
            << "Image:                 " << options.image << '\n'
            << "Timing:                " << options.timing << '\n'
            << "Seam metrics:          " << seam_csv << '\n';
#else
        std::cout
