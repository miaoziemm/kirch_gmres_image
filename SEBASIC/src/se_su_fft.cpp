...
su_fft_npfa		return valid n for std::complex<float>-to-std::complex<float> PFA
su_fft_npfar		return valid n for real-to-std::complex<float>/std::complex<float>-to-real PFA
su_fft_npfao		return optimal n for std::complex<float>-to-std::complex<float> PFA
se_su_fft_npfaro		return optimal n for real-to-std::complex<float>/std::complex<float>-to-real PFA
su_fft_pfacc		1D PFA std::complex<float> to std::complex<float>
su_fft_pfacr		1D PFA std::complex<float> to real
su_fft_pfarc		1D PFA real to std::complex<float>
su_fft_pfamcc		multiple PFA std::complex<float> to real
su_fft_pfa2cc		2D PFA std::complex<float> to std::complex<float>
su_fft_pfa2cr		2D PFA std::complex<float> to real
su_fft_pfa2rc		2D PFA real to std::complex<float>
...
int se_su_fft_npfaro (int nmin, int nmax);
...
Returned:	valid n for real-to-std::complex<float>/std::complex<float>-to-real prime factor fft
...
z		array[n] of std::complex<float> numbers to be transformed in place

Output:
z		array[n] of std::complex<float> numbers transformed
...
isign       sign of isign is the sign of exponent in fourier kernel
n           length of transform (see notes below)
cz          array[n/2+1] of std::complex<float> values (may be equivalenced to rz)
...
Input:
isign       	sign of isign is the sign of exponent in fourier kernel
n           	number of std::complex<float> elements per transform (see notes below)
nt          	number of transforms
k           	stride in std::complex<float> elements within transforms
kt          	stride in std::complex<float> elements between transforms
z           	array of std::complex<float> elements to be transformed in place

Output:
z		array of std::complex<float> elements transformed
...
z           	array[n2][n1] of std::complex<float> elements to be transformed in place

Output:
z		array[n2][n1] of std::complex<float> elements transformed
...
n1          1st (fast) dimension of array to be transformed (see notes)
n2          2nd (slow) dimension of array to be transformed (see notes)
cz          array of std::complex<float> values (may be equivalenced to rz)
...
su_fft_npfar and se_su_fft_npfaro:
Current implemenations of real-to-std::complex<float> and std::complex<float>-to-real prime 
factor ffts require that the transform length n be even and that n/2 
be a valid length for a std::complex<float>-to-std::complex<float> prime factor fft.  The 
value returned by su_fft_npfar satisfies these conditions.  Also, see notes 
for su_fft_npfa.
...
su_fft_pfacr:
Because su_fft_pfacr uses su_fft_pfacc to do most of the work, n must be even 
and n/2 must be a valid length for su_fft_pfacc.  The simplest way to
obtain a valid n is via n = su_fft_npfar(nmin).  A more optimal n can be 
obtained with se_su_fft_npfaro.

su_fft_pfarc:
Because su_fft_pfarc uses su_fft_pfacc to do most of the work, n must be even 
and n/2 must be a valid length for su_fft_pfacc.  The simplest way to
obtain a valid n is via n = su_fft_npfar(nmin).  A more optimal n can be 
obtained with se_su_fft_npfaro.

su_fft_pfamcc:
To perform a two-dimensional transform of an n1 by n2 std::complex<float> array 
(assuming that both n1 and n2 are valid "n"), stored with n1 fast 
and n2 slow:
    su_fft_pfamcc(isign,n1,n2,1,n1,z); (to transform 1st dimension)
    su_fft_pfamcc(isign,n2,n1,n1,1,z); (to transform 2nd dimension)

su_fft_pfa2cc:
Only one (either the 1st or 2nd) dimension of the 2-D array is transformed.

If idim equals 1, then n2 transforms of n1 std::complex<float> elements are performed; 
else, if idim equals 2, then n1 transforms of n2 std::complex<float> elements are 
performed.
...
To perform a two-dimensional transform of an n1 by n2 std::complex<float> array 
(assuming that both n1 and n2 are valid "n"), stored with n1 fast 
and n2 slow:  su_fft_pfa2cc(isign,1,n1,n2,z);  su_fft_pfa2cc(isign,2,n1,n2,z);

su_fft_pfa2cr:
If idim equals 1, then n2 transforms of n1/2+1 std::complex<float> elements to n1 real 
elements are performed; else, if idim equals 2, then n1 transforms of n2/2+1 
std::complex<float> elements to n2 real elements are performed.
...
n2 or an n1 by n2/2+1 two-dimensional array of std::complex<float> elements.

Let n denote the transform length, either n1 or n2, depending on idim.
Because su_fft_pfa2rc uses su_fft_pfa2cc to do most of the work, n must be even 
and n/2 must be a valid length for su_fft_pfa2cc.  The simplest way to
obtain a valid n is via n = su_fft_npfar(nmin).  A more optimal n can be 
obtained with se_su_fft_npfaro.

su_fft_pfa2rc:
If idim equals 1, then n2 transforms of n1 real elements to n1/2+1 std::complex<float> 
elements are performed; else, if idim equals 2, then n1 transforms of n2 
real elements to n2/2+1 std::complex<float> elements are performed.
...
int su_fft_npfar (int nmin)
/*****************************************************************************
Return smallest valid n not less than nmin for real-to-std::complex<float> or 
std::complex<float>-to-real prime factor ffts.
******************************************************************************
Input:
nmin		lower bound on returned value
...
*****************************************************************************/
{
    return 2*su_fft_npfa((nmin+1)/2);
}

int se_su_fft_npfaro (int nmin, int nmax)
/*****************************************************************************
Return optimal n between nmin and nmax for real-to-std::complex<float> or 
std::complex<float>-to-real prime factor ffts
******************************************************************************
Input:
nmin		lower bound on returned value
...
factor ffts require that the transform length n be even and that n/2 
be a valid length for a std::complex<float>-to-std::complex<float> prime factor fft.  The 
value returned by se_su_fft_npfaro satisfies these conditions.  Also, see notes 
for su_fft_npfao.
******************************************************************************
Author:  Dave Hale, Colorado School of Mines, 06/16/89
*****************************************************************************/
{
    return 2*su_fft_npfao((nmin+1)/2,(nmax+1)/2);
}
...
void su_fft_pfacc (int isign, int n, std::complex<float> cz[])
/*****************************************************************************
Prime factor fft:  std::complex<float> to std::complex<float> transform, in place
******************************************************************************
Input:
isign		sign of isign is the sign of exponent in fourier kernel
n		length of transform (see notes below)
...
void su_fft_pfacr (int isign, int n, std::complex<float> cz[], float rz[])
/*****************************************************************************
Prime factor fft:  std::complex<float> to real transform
******************************************************************************
Input:
isign       sign of isign is the sign of exponent in fourier kernel
n           length of transform (see notes below)
...
Also, see notes and references for function su_fft_pfacc.
******************************************************************************
Author:  Dave Hale, Colorado School of Mines, 06/13/89
*****************************************************************************/
{
    ...
    /* do std::complex<float> to std::complex<float> transform */
    su_fft_pfacc(isign,n/2,(std::complex<float>*)z);
}

void su_fft_pfarc (int isign, int n, float rz[], std::complex<float> cz[])
/*****************************************************************************
Prime factor fft:  real to std::complex<float> transform
******************************************************************************
Input:
isign       sign of isign is the sign of exponent in fourier kernel
n           length of transform; must be even (see notes below)
...
void su_fft_pfamcc (int isign, int n, int nt, int k, int kt, std::complex<float> cz[])
/*****************************************************************************
Prime factor fft:  multiple std::complex<float> to std::complex<float> transforms, in place
******************************************************************************
Input:
isign       	sign of isign is the sign of exponent in fourier kernel
n           	number of std::complex<float> elements per transform (see notes below)
...
where
    0 <= p <= 4,  0 <= q <= 2,  0 <= r,s,t,u <= 1
is required for su_fft_pfamcc to yield meaningful results.  this
restriction implies that n is restricted to the range
    1 <= n <= 720720 (= 5*7*9*11*13*16)
...
void su_fft_pfa2cc (int isign, int idim, int n1, int n2, std::complex<float> cz[])
/*****************************************************************************
Prime factor fft:  2-D std::complex<float> to std::complex<float> transforms, in place
******************************************************************************
Input:
isign       	sign of isign is the sign of exponent in fourier kernel
idim        	dimension to transform, either 1 or 2 (see notes)
...
where
    0 <= p <= 4,  0 <= q <= 2,  0 <= r,s,t,u <= 1
is required for su_fft_pfa2cc to yield meaningful results.  this
restriction implies that n is restricted to the range
    1 <= n <= 720720 (= 5*7*9*11*13*16)
...
*****************************************************************************/
{
    ...
    /* do multiple std::complex<float> to std::complex<float> transforms */
    su_fft_pfamcc(isign,n,nt,k,kt,cz);
}

void su_fft_pfa2cr (int isign, int idim, int n1, int n2, std::complex<float> cz[], float rz[])
/*****************************************************************************
Prime factor fft:  2-D std::complex<float> to real transforms
******************************************************************************
Input:
isign       sign of isign is the sign of exponent in fourier kernel
idim        dimension to transform, which must be either 1 or 2 (see notes)
...
Also, see notes and references for function su_fft_pfa2cc.
******************************************************************************
Author:  Dave Hale, Colorado School of Mines, 06/13/89
...
*****************************************************************************/
{
    ...
    if (idim==1) {
        ...
        /* transform as std::complex<float> elements */
        su_fft_pfa2cc(isign,1,n1/2,n2,(std::complex<float>*)z);

    /* else, if transforming dimension 2 */
    } else {

        /* transform as std::complex<float> elements */
        su_fft_pfa2cc(isign,2,n1,n2/2,(std::complex<float>*)z);

        /* unmerge even and odd vectors */
        temp = (float*)alloc1(n1, sizeof(float));
        for (i2=0; i2<n2; i2+=2) {
...
void su_fft_pfa2rc (int isign, int idim, int n1, int n2, float rz[], std::complex<float> cz[])
/*****************************************************************************
Prime factor fft:  2-D real to std::complex<float> transforms
******************************************************************************
Input:
isign       sign of isign is the sign of exponent in fourier kernel
idim        dimension to transform, which must be either 1 or 2 (see notes)
...
*****************************************************************************/
{
    ...
    if (idim==1) {
        ...
        /* transform as std::complex<float> elements */
        su_fft_pfa2cc(isign,1,n1/2,n2,cz);
    ...
    /* else, if transforming dimension 2 */
        ...
        /* transform as std::complex<float> elements */
        su_fft_pfa2cc(isign,2,n1,n2/2,cz);
