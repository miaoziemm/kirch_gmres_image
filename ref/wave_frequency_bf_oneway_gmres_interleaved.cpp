                      ...
                      bool block_metrics)
{
    ...
    stream << ",total_change_from_initial,step_change,seam_ratio_real,"
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        const std::string initial_output_real = huygens_cli::optional_string(
            "initial_output_real", "wave_interleaved_initial_real.rsf");
        const std::string initial_output_imag = huygens_cli::optional_string(
            "initial_output_imag", "wave_interleaved_initial_imag.rsf");
        const std::string output_prefix = huygens_cli::optional_string(
            "output_prefix", "wave_interleaved");
        const std::string summary_csv = huygens_cli::optional_string(
            "summary_csv", "interleaved_summary.csv");
        const std::string block_metrics_csv = huygens_cli::optional_string(
            "block_metrics_csv", "interleaved_block_updates.csv");
        const std::string propagation_csv = huygens_cli::optional_string(
            "propagation_csv", "interleaved_propagation.csv");
        ...
        if (!(frequency > 0.0f) || !(filter_dt > 0.0f) ||
            ...
            p < 2 || leaf < 4 || p >= leaf || panel_levels < 0 ||
            !(phase_tol > 0.0f) || amp_eps < 0.0f || iterations < 1 ||
            restart < 1 || gmres_cycles < 1 || !(tolerance > 0.0) ||
            shift < 0.0 || ilut_drop_tolerance < 0.0 || ilut_fill_factor < 1 ||
            minimum_relaxation > maximum_relaxation || correction_sigma < 0.0 ||
            clip_fraction < 0.0 || clip_percentile < 0.0 ||
        ...
        if (source_iz != 0) {
            throw std::invalid_argument("source_iz must be zero");
        }
        ...
        std::cout
            << "Interleaved phase-aware ButterflyPACK + SuperLU-ILUT/Eigen-GMRES\n"
            << "Ordering per sweep: C1 -> K2 -> C2 -> K3 -> C3 -> ...\n";

        std::vector<ComplexF> initial_float;
        se::huygens::initialize_first_block_hankel_one_way(
            model, info, frequency, source_ix, source_iz,
            source_amplitude, source_radius, initial_float);
        const std::vector<ComplexD> initial = to_double(initial_float);
        std::vector<ComplexD> current = initial;
        std::vector<ComplexD> previous = initial;
        const double initial_norm = std::max(
            vector_norm(initial), std::numeric_limits<double>::min());
        const double omega = 2.0 * kPi * static_cast<double>(frequency);
        const float omega_float = static_cast<float>(omega);

        ensure_parent(initial_output_real);
        ensure_parent(initial_output_imag);
        se::huygens::write_wavefield_component_rsf(
            initial_output_real, model, initial_float, frequency,
            "interleaved_initial_hankel_first_block", false);
        se::huygens::write_wavefield_component_rsf(
            initial_output_imag, model, initial_float, frequency,
            "interleaved_initial_hankel_first_block", true);
        ...
        for (std::size_t iblock = 0; iblock < info.blocks.size(); ++iblock) {
            ...
            system->matrix = build_local_operator(
                model, system->block.source_iz,
                system->block.target_end_iz, omega,
                system->nx_unknown, system->nz_unknown);
            ...
            system->solver->preconditioner().configure(
                shift, ilut_drop_tolerance, ilut_fill_factor);
        ...
        std::ofstream propagation_stream(propagation_csv);
        if (!summary_stream || !block_stream || !propagation_stream) {
            throw std::runtime_error("failed to open experiment CSV outputs");
        }
        ...
        propagation_stream << std::setprecision(17)
            << "iteration,block,build_seconds,apply_seconds\n";
        ...
        auto correct_block = [&](int iteration,
            ...
            relaxation = std::clamp(
                relaxation, minimum_relaxation, maximum_relaxation);
            correction *= relaxation;

            std::vector<ComplexD> correction_grid = vector_to_grid(
                correction, system.nz_unknown, system.nx_unknown);
            gaussian_filter_in_place(
                correction_grid, system.nz_unknown,
                system.nx_unknown, correction_sigma);
            ...
            for (int iz = 0; iz < system.nz_unknown; ++iz) {
                for (int ix = 0; ix < system.nx_unknown; ++ix) {
                    ...
                    const double limit = clip_fraction * std::max(
                        amplitude[correction_index], amplitude_floor);
            ...
            const Vector corrected_guess = interior_vector(
                corrected_local, local_nz, model.nx,
                system.nx_unknown, system.nz_unknown);
            ...
            const std::vector<ComplexD> guess_grid = vector_to_grid(
                guess, system.nz_unknown, system.nx_unknown);
            ...
            const std::vector<ComplexD> current_local =
                extract_local(current, model.nx, top, bottom);
            const double changed_from_input =
                difference_norm(current_local, local) /
                std::max(vector_norm(local),
                         std::numeric_limits<double>::min());
            ...
            block_stream
                ...
                << residual_ratio << ',' << changed_from_input << '\n';
        };

        auto propagate_to_block = [&](int iteration, std::size_t iblock) {
...
#pragma omp parallel for schedule(dynamic, 1)
            ...
            for (int iz_local = 0; iz_local < target_depths; ++iz_local) {
                ...
                factors[static_cast<std::size_t>(iz_local)].reset(
                    bf1d_strict_segmented_create_phase_amp(
                        ...
                        omega_float, p, leaf, panel_levels, amp_eps, phase_tol));
            ...
            std::vector<ComplexF> current_float = to_float(current);
            const std::vector<ComplexF> boundary_input =
                se::huygens::gather_boundary_values(
                    model, block, geometry.source_ix, current_float);
            FFTStorage packed_input;
            copy_to_fftw(boundary_input, packed_input);
            std::vector<ComplexF> target_values(geometry.targets.size());
...
#pragma omp parallel for schedule(dynamic, 1)
            ...
            for (int iz_local = 0; iz_local < target_depths; ++iz_local) {
                ...
                for (int ix_target = 0; ix_target < model.nx; ++ix_target) {
                    const std::size_t row =
                        static_cast<std::size_t>(ix_target) * target_depths + iz_local;
                    target_values[row] = ComplexF(
            ...
            se::huygens::inject_target_values(
                model, geometry.targets, target_values, current_float);
            current = to_double(current_float);

            propagation_stream << iteration << ',' << (iblock + 1) << ','
                               << build_seconds << ',' << apply_seconds << '\n';
            std::cout << "iteration=" << iteration
                      << " propagated block=" << (iblock + 1)
                      << " build=" << build_seconds
        ...
        for (int iteration = 1; iteration <= iterations; ++iteration) {
            ...
            correct_block(iteration, 0, block_updates, block_ratios);
            for (std::size_t iblock = 1; iblock < info.blocks.size(); ++iblock) {
                propagate_to_block(iteration, iblock);
                correct_block(iteration, iblock, block_updates, block_ratios);
            }

            const double total_change =
                difference_norm(current, initial) / initial_norm;
            const double step_change =
                difference_norm(current, previous) / initial_norm;
            ...
            se::huygens::write_wavefield_component_rsf(
                ...
                model, current_float, frequency,
                "interleaved_butterfly_then_local_gmres", false);
            se::huygens::write_wavefield_component_rsf(
                indexed_filename(output_prefix, iteration, "imag"),
                model, current_float, frequency,
                "interleaved_butterfly_then_local_gmres", true);
            previous = current;

            std::cout << "interleaved iteration=" << iteration
        ...
        std::cout << "Interleaved ordering experiment completed.\n"
                  << "Summary CSV: " << summary_csv << '\n'
                  << "Block CSV: " << block_metrics_csv << '\n'
                  << "Propagation CSV: " << propagation_csv << '\n';
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "wave_frequency_bf_oneway_gmres_interleaved: "
                  << error.what() << '\n';
        return 1;
    }
}
