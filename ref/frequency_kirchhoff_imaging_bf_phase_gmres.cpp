...
#include "matlab_phase_gmres.hpp"
...
void correct_complete_block(
    ...
    float frequency, const matlab_phase_gmres::Parameters& parameters,
    ...
    const char* field_name, std::ofstream& metrics_stream)
{
    ...
    if (static_cast<int>(values.size()) != nz * nx) {
        throw std::invalid_argument("phase correction requires one complete frequency block");
    }
    std::vector<float> velocity(static_cast<std::size_t>(nz) * nx);
    matlab_phase_gmres::Vector initial(nz * nx);
    for (int ix = 0; ix < nx; ++ix) {
        for (int iz = 0; iz < nz; ++iz) {
            const int mi = matlab_phase_gmres::index(iz, ix, nz);
    ...
    std::vector<matlab_phase_gmres::IterationMetric> metrics;
    const auto corrected = matlab_phase_gmres::correct(
    ...
    for (const auto& m : metrics) {
        metrics_stream << shot << ',' << block << ',' << frequency << ','
            ...
            << m.residual_before << ',' << m.residual_after_phase << ','
            << m.residual_after_gmres << ',' << m.phase_correction_norm << ','
            << m.gmres_correction_norm << ',' << m.alpha.real() << ','
            << m.alpha.imag() << ',' << static_cast<int>(m.phase_accepted)
    ...
    try {
        ...
        const fki::CommonOptions options =
            fki::parse_common_options(
                "frequency_kirchhoff_imaging_bf_phase_gmres");
        ...
        const int requested_depth_batch =
            huygens_cli::optional_int(
                "bf_depth_batch", 1);
        matlab_phase_gmres::Parameters correction;
        correction.outer_iterations = huygens_cli::optional_int("phase_gmres_outer", 3);
        correction.coarse_outer_max = huygens_cli::optional_int("phase_coarse_outer_max", 3);
        correction.q_coarse = huygens_cli::optional_int("phase_q_coarse", 2);
        ...
        correction.absorbing_rows = huygens_cli::optional_int("phase_nabs", 6);
        correction.gmres_tolerance = huygens_cli::optional_float("gmres_tolerance", 1.0e-12f);
        correction.alpha_cap = huygens_cli::optional_float("phase_alpha_cap", 0.8f);
        correction.phase_amp_rel_floor = huygens_cli::optional_float("phase_amp_rel_floor", 1.0e-4f);
        correction.damp_max = huygens_cli::optional_float("phase_damp_max", 2.0f);
        const std::string correction_csv = huygens_cli::optional_string(
            "correction_csv", "phase_gmres_metrics.csv");
        ...
        if (correction.outer_iterations != 3 || correction.coarse_outer_max < 0 ||
            correction.coarse_outer_max > correction.outer_iterations ||
            correction.q_coarse < 1 || correction.gmres_restart < 1 ||
            correction.gmres_cycles < 1 || correction.absorbing_rows < 0 ||
            !(correction.gmres_tolerance > 0.0) || !(correction.alpha_cap > 0.0) ||
            !(correction.phase_amp_rel_floor > 0.0) || !(correction.damp_max >= 0.0)) {
            throw std::invalid_argument(
                "invalid MATLAB phase+GMRES parameters; phase_gmres_outer must equal 3");
        }
        ...
        correction_stream << std::setprecision(17)
            << "shot,block,frequency,field,outer,gmres_steps,residual_before,"
               "residual_after_phase,residual_after_gmres,phase_correction_norm,"
               "gmres_correction_norm,alpha_real,alpha_imag,phase_accepted,gmres_accepted\n";
...
#pragma omp for schedule(dynamic, 1)
        ...
        std::cout
            << "Block BF + MATLAB phase-coarse GMRES imaging completed.\n"
