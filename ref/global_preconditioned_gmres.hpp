                                         ...
                                         int transition_rows)
{
    ...
    const int rows = std::clamp(transition_rows, 1, std::max(1, system.nz - 2));
    Vector chi = Vector::Ones(receiver_initial.size());
    for (int iz = 0; iz < rows; ++iz) {
        const double value = static_cast<double>(iz) /
            static_cast<double>(rows);
        for (int ix = 0; ix < system.nx; ++ix) {
            chi[index(iz, ix, system.nx)] = Complex(value, 0.0);
        }
    }
...
{
    const int rows = std::clamp(transition_rows, 1, std::max(1, system.nz - 2));
    Vector x = receiver_initial;
    for (int iz = 0; iz < rows; ++iz) {
        const double value = static_cast<double>(iz) /
            static_cast<double>(rows);
        for (int ix = 0; ix < system.nx; ++ix)
            x[index(iz, ix, system.nx)] *= value;
    }
    return x;
}
