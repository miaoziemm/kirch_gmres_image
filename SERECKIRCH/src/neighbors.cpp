...
/* find new traveltime at gridpoint i */
{
    ...
    if(v[2]->value < FLT_MAX) {   /* ALL THREE DIRECTIONS CONTRIBUTE */
	if (updaten(3, &res, v) || 
	    updaten(2, &res, v) || 
	    updaten(1, &res, v)) return res;

    } else if(v[1]->value < FLT_MAX) { /* TWO DIRECTIONS CONTRIBUTE */
	if (updaten(2, &res, v) || 
	    updaten(1, &res, v)) return res;

    } else if(v[0]->value < FLT_MAX) { /* ONE DIRECTION CONTRIBUTES */
...
/*< initialize the source in the spherical case (rtp), added by YC, Apr, 2022 >*/
{
    ...
    /* loop in a small box around the source */
    npoints = n[0]*n[1]*n[2];
    WARN(("npoints before is %d",npoints));
    for (ix=start[2]; ix <= endx[2]; ix++) {
	for (iy=start[1]; iy <= endx[1]; iy++) {
	    for (iz=start[0]; iz <= endx[0]; iz++) {
		npoints--;
...
/*		delta[1] = xs[1]-iy*d[1];
    ...
    WARN(("npoints after is %d",npoints));
    return npoints;
}
