...
!>**** C interface of matrix construction
   ...
   subroutine C_BPACK_Construct_Element_Compute(bmat_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncZmn, C_FuncZmnBlock, C_QuantApp) bind(c, name="d_c_bpack_construct_element_compute")
      implicit none
...
!>**** C interface of matrix construction via entry evaluation
   ...
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BPACK_Construct_Init(N, Ndim, Locations, nns, nlevel, tree, Permutation, N_loc, bmat_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncDistmn, C_FuncNearFar, C_QuantApp) bind(c, name="d_c_bpack_construct_init")
      implicit none
      integer N, Ndim
...
!>**** C interface of BPACK factorization
   ...
   !> @param ptree_Cptr: the structure containing process tree
   !> @param msh_Cptr: the structure containing points and ordering information (in)
   subroutine C_BPACK_Factor(bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr, msh_Cptr) bind(c, name="d_c_bpack_factor")
      implicit none
...
!>**** C interface of BPACK solve
   ...
   !> @param stats_Cptr: the structure containing statistics
   !> @param ptree_Cptr: the structure containing process tree
   subroutine C_BPACK_Solve(x, b, Nloc, Nrhs, bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr) bind(c, name="d_c_bpack_solve")
      implicit none
...
!>**** C interface of a blackbox tfqmr without preconditioner, or assuming preconditioner is applied in the blackbox matvec
   ...
   !> @param ker_Cptr: the structure containing kernel quantities
   !> @param C_FuncHMatVec: the C_pointer to user-provided function to multiply A and A* with vectors (in)
   subroutine C_BPACK_TFQMR_Noprecon(x, b, Nloc, Nrhs, option_Cptr, stats_Cptr, ptree_Cptr, ker_Cptr, C_FuncHMatVec, C_QuantApp) bind(c, name="d_c_bpack_tfqmr_noprecon")
      implicit none
...
!>**** C interface of a blackbox tfqmr without preconditioner, or assuming preconditioner is applied in the blackbox matvec (tensor)
   ...
   subroutine C_BPACK_MD_TFQMR_Noprecon(Ndim, x, b, Nloc, Nrhs, option_Cptr, stats_Cptr, ptree_Cptr, ker_Cptr, C_FuncHMatVec_MD, C_QuantApp) bind(c, name="d_c_bpack_md_tfqmr_noprecon")
      implicit none
      integer Ndim
      integer Nloc(Ndim), Nrhs
...
!>**** C interface of BPACK-vector multiplication
   ...
   !> @param ptree_Cptr: the structure containing process tree
   !> @param trans: 'N', 'C' or 'T'
   subroutine C_BPACK_Mult(trans, xin, xout, Ninloc, Noutloc, Ncol, bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr) bind(c, name="d_c_bpack_mult")
      implicit none
      real(kind=8) t1, t2
      integer Ninloc, Noutloc, Ncol
      DT::xin(Ninloc, Ncol), xout(Noutloc, Ncol)
...
!>**** C interface of BPACK log-determinant
   ...
   !> @param bmat_Cptr: the structure containing BPACK
   !> @param option_Cptr: the structure containing option
   subroutine C_BPACK_Logdet(phase, logabsdet, option_Cptr, bmat_Cptr) bind(c, name="d_c_bpack_logdet")
      implicit none
      DTR:: logabsdet
      DT:: phase
      type(c_ptr), intent(in) :: bmat_Cptr
