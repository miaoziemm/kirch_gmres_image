...
BIN="$ROOT/build/bin"
MODEL="$ROOT/model/vmar.rsf"
OUT="$ROOT/image_test/mar_single_shot_point_source_gmres"
...
"$BIN/sewave2d_forward" \
    ...
    nt=6000 dt=0.0007 fdom=20 \
    sx="$SOURCE_X" sz="$SOURCE_Z" rz=0 \
    ns=1 ds=0 nr=1024 r0=0 dr=0.0105 \
    nbc=40 L=30 alpha=1 \
...
"$BIN/frequency_kirchhoff_imaging_bf_global_gmres" \
    ...
    nfft=8192 fmin="$FMIN" fmax="$FMAX" frequency_stride="$FREQUENCY_STRIDE" \
    source_stride=1 target_x_stride=1 target_z_stride=1 \
    threads="$THREADS" \
